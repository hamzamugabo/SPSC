    ...
    if request.method == 'POST':
        try:
            ...
            with sql.connect("sample_db.sqlite") as con:
                ...
                cur.execute("INSERT INTO user (fname,lname,email,tellnumber,username,password) VALUES(?, ?, ?, ?, ?, ?)",(fname, lname, email, tell, username, hash_password))
...
cur_dir = os.path.dirname(__file__)  #this shows the directory from which the file loads.
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            df = pd.read_csv(file_path, encoding='ISO-8859-1', sep=',', delimiter=None, header=0, index_col=None,
                             squeeze=False, engine='python', usecols=range(0, 2),na_values=missing_values)
...
class RegistrationForm(form.Form):
    ...
    #mobile_phone = StringField(validators=([validators.DataRequired(), regexp("\d{3}[ ,-]?\d{3}[ ,-]?\d{4}")]))
    #firstname = StringField("validators=[validators.DataRequired("Please enter your  first name.")])
    #lastname = StringField("validators=validators=[validators.DataRequired("Please enter your last name."),
    #                                                     validators.Length(max=255)])
...
# Create admin
admin = admin.Admin(app, 'Social Media Post Sentiment Admin panel', index_view=MyAdminIndexView(), base_template='my_master.html')
