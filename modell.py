...
df = pd.read_csv('F:/final.csv', encoding='latin', sep=',', delimiter=None, header=0,
...
pipeline_one = Pipeline([
    ...
    ('feature-extractors', FeatureUnion([('tfidf', TfidfVectorizer()),
                                        ('counts', CountVectorizer(analyzer='word', tokenizer=tokenize, lowercase=True,
                                                                   ...
                                                                   max_features=1500, min_df=4, max_df=0.9))]))])
...
X_train, X_test, y_train, y_test = train_test_split(features, y, train_size=0.8, test_size=0.2, random_state=1234)
...
# train classifier
log_model = LogisticRegression(solver='lbfgs')
log_model = log_model.fit(X=features, y=y)
